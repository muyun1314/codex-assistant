...
export function normalizeMessages(messages, { coerceStrings = false } = {}) {
  ...
  // Pass 3: drop orphan tool messages and empty assistant messages.
  // A tool message is valid only when its tool_call_id matches an unresolved id from the nearest prior assistant tool call.
  // An assistant message is valid only when it has content or tool_calls.
  const validated = [];
  ...
  for (const msg of merged) {
    if (msg.role === "assistant" && Array.isArray(msg.tool_calls)) {
      ...
      const hasContent = msg.content && (typeof msg.content === 'string' ? msg.content.trim() : msg.content.length > 0);
      if (hasContent || msg.reasoning_content) {
        validated.push(msg);
      }
      // If empty, skip it (don't add to validated)
      pendingToolCallIds.clear();
    } else if (msg.role === "tool") {
