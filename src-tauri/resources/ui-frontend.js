...
// Simple semver comparison for frontend
...
async function _checkUpdateFromBrowser() {
  ...
  try {
    var res = await fetch(GITHUB_API, { headers: { 'Accept': 'application/vnd.github.v3+json' } });
...
async function checkForUpdates() {
  ...
  try {
    ...
    if (data.hasUpdate) {
      ...
      // Server check failed — try from browser context
      ...
      if (browserResult.hasUpdate) {
        ...
        toast('检查更新失败：' + data.checkError + '；浏览器直连也失败', 'error');
      } else {
