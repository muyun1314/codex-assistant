...
// Try GitHub API from the frontend (browser context respects system proxy / hosts file)
...
async function checkForUpdates() {
  ...
  try {
    ...
    if (data.hasUpdate) {
      ...
      // Server check failed — try from browser context (hosts file / system proxy work here)
      console.log('[ui] Server update check failed (' + data.checkError + '), trying from browser...');
      var browserResult = await _checkUpdateFromBrowser();
      if (browserResult.hasUpdate) {
        showUpdateModal(browserResult);
        if (btn) btn.classList.add('has-update');
      } else if (browserResult.error) {
        toast('检查更新失败：' + data.checkError + '；浏览器直连也失败：' + browserResult.error, 'error');
      } else {
    ...
    // API call itself failed — try browser directly
    console.log('[ui] API call failed (' + e.message + '), trying from browser...');
    var browserResult = await _checkUpdateFromBrowser();
    if (browserResult.hasUpdate) {
      showUpdateModal(browserResult);
      if (btn) btn.classList.add('has-update');
    } else if (browserResult.error) {
      toast('检查更新失败：' + browserResult.error, 'error');
    } else {
      toast('当前已是最新版本 v' + (currentVersionInfo ? currentVersionInfo.version : '?'));
    }
    if (btn && !browserResult.hasUpdate) btn.classList.remove('has-update');
  } finally {
    if (btn) {
      btn.disabled = false;
