...
function getCodexInstallInfo() {
  // 检查微软商店版
  try {
    const storeResult = execFileSync('powershell', ['-Command', 'Get-AppxPackage -Name "*Codex*" | Select-Object -ExpandProperty PackageFamilyName'], { encoding: 'utf8', timeout: 5000 }).trim();
    if (storeResult && storeResult.includes('OpenAI.Codex')) {
      return { type: 'store', packageFamilyName: storeResult };
    }
  } catch {}
...
function searchExecutable(name, altNames) {
  ...
  for (var wn = 0; wn < allExeNames.length; wn++) {
    try {
      var whereOutput = execFileSync('where', [allExeNames[wn]], { encoding: 'utf8', timeout: 5000 }).trim();
      if (whereOutput) {
  ...
  // Level 5: Deep recursive search in common install roots (PowerShell — fast, only first 5 matches)
  try {
    var deepRoots = [];
...
function getCodexPlusPlusPath() {
  ...
  if (manual.codexppPath && fs.existsSync(manual.codexppPath)) return manual.codexppPath;
  // Windows filenames cannot contain '+', so try common variations
  return searchExecutable('codex-plus-plus', [
    'codexpp',
    'codexplusplus',
    'CodexPlusPlus',
    'CodexPP',
...
function getCodexPlusPlusManagerPath() {
  // Check manual config first
  var manual = loadCodexppManualConfig();
  if (manual.codexppMgrPath && fs.existsSync(manual.codexppMgrPath)) return manual.codexppMgrPath;
  return searchExecutable('codex-plus-plus-manager', [
    'codexpp-manager',
...
`;
  ...
  if (pathname === '/api/codexpp-path' && method === 'GET') {
    try {
      ...
      // Fall back to auto-detected if no manual config
      if (!codexppCfg.codexppPath) {
        var autoPath = getCodexPlusPlusPath();
        if (autoPath) codexppCfg.codexppPath = autoPath;
      }
      if (!codexppCfg.codexppMgrPath) {
        var autoMgr = getCodexPlusPlusManagerPath();
        if (autoMgr) codexppCfg.codexppMgrPath = autoMgr;
      }
