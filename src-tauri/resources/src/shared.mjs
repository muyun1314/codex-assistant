...
export function latestUserMessageHasUrl(messages) {
  for (let i = messages.length - 1; i >= 0; i--) {
    const message = messages[i];
    if (message?.role === "user") return contentHasUrl(message.content);
  }
  return false;
}
...
export function wireClientCancel(res, upstreamRes) {
  ...
  const onClose = () => {
    ...
    cancelled = true;
    try {
      // stream 被锁定（正在 for-await 读取）时 cancel 会抛 ERR_INVALID_STATE
      if (upstreamRes.body.locked) return;
...
export function writeWithBackpressure(res, chunk, timeoutMs = 30000) {
  if (!res || res.destroyed || res.closed) return Promise.resolve();
  try {
...
export async function fetchModelsFromAPI(baseUrl, apiKey) {
  ...
  for (const url of candidates) {
    try {
      ...
      // 401 means bad API key — collect error but still try other candidates
      // (some providers may have different auth requirements per endpoint)
      if (statusCode === 401) {
        ...
        errors.push(url + ' -> HTTP 401: ' + authMsg);
        continue;
      }
