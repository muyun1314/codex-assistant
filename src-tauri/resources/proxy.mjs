...
import { touchResponse, storeResponse, resolveResponseChain, reasoningIndex as storeReasoningIndex } from "./src/store.mjs";
...
// SIGINT/SIGTERM handled by gracefulShutdown below
...
// DeepSeek V4 默认会开�?thinking mode。Codex / Codex Assistant 目前不会稳定回传
// DeepSeek 要求�?`reasoning_content`，所以默认强制关�?DeepSeek thinking�?
...
function loadDynamicProviders() {
  try {
    ...
    for (var i = 0; i < (data.providers || []).length; i++) {
        ...
        if (result.key !== null) {
          ...
          if (proxyAuthKey) {
            ...
            if (fallback.key !== null) {
              ...
              console.error('[proxy] ⚠️ 无法解密提供商 "' + p.name + '" 的 API Key — 加密环境可能已改变。请在 UI 中重新输入 API Key。');
              decryptFailed = true;
      ...
      // 即使解密失败也注册提供商（用空 key），让代理能启动
      // 实际 API 调用会失败，但用户可以通过 UI 重新输入 key
      if (!apiKey && !decryptFailed) continue;
      ...
      providers[name] = {
        ...
        _decrypt_failed: decryptFailed || false,
      };
    }
    
    if (hasDecryptErrors) {
      console.error('[proxy] ═══════════════════════════════════════════════════');
      console.error('[proxy] ⚠️  检测到 API Key 解密失败！');
      console.error('[proxy]    原因：加密环境发生变化（可能换了电脑或重装了系统）');
      console.error('[proxy]    解决：请在 UI 的「提供商管理」页面重新输入 API Key');
      console.error('[proxy] ═══════════════════════════════════════════════════');
    }
    
    return { providers: providers, providerNames: providerNames, hasDecryptErrors: hasDecryptErrors };
  } catch (err) {
    console.error('[Codex Assistant] Failed to load user/provider-configs.json:', err.message);
    return { providers: {}, providerNames: [], hasDecryptErrors: false };
  }
}
...
function buildProviders() {
  // 静�?provider 仅作�?.env 配置的回退，且只有配置�?key 的才会加�?
  // 所有通过 UI 配置�?provider 来自 provider-configs.json（动态）
  var staticProviders = {};
  if (DEEPSEEK_KEY) {
...
for (var [name, cfg] of Object.entries(OAI_COMPAT_PROVIDERS)) {
  var status = cfg._decrypt_failed ? '[解密失败]' : (cfg.key ? '[key]' : '[no key]');
  console.log(`  ${name}: ${cfg.models.length} models, base=${cfg.base} ${status}`);
}

var enabledProviders = new Set();
for (var [name, cfg] of Object.entries(OAI_COMPAT_PROVIDERS)) {
  if (cfg.key || cfg._decrypt_failed) enabledProviders.add(name);
}
...
var responseStore = new Map();
var reasoningIndex = storeReasoningIndex; // callId -> reasoningContent, shared incremental index for O(1) lookup
var MAX_CONSECUTIVE_TOOL_CALLS = Number(process.env.MAX_CONSECUTIVE_TOOL_CALLS) || 20; // circuit breaker threshold
...
function getFallbackProvider() {
  // 优先返回有有效 key 的提供商
  if (DEFAULT_PROVIDER && enabledProviders.has(DEFAULT_PROVIDER)) {
    var cfg = OAI_COMPAT_PROVIDERS[DEFAULT_PROVIDER];
    if (cfg && cfg.key && !cfg._decrypt_failed) return DEFAULT_PROVIDER;
  }
  if (enabledProviders.has("openai")) return "openai";
  // 找第一个有有效 key 的提供商
  for (var name of Object.keys(OAI_COMPAT_PROVIDERS)) {
    if (enabledProviders.has(name)) {
      var c = OAI_COMPAT_PROVIDERS[name];
      if (c && c.key && !c._decrypt_failed) return name;
    }
  }
  // 所有提供商的 key 都解密失败，仍然返回一个以允许代理启动
  // 用户可以通过 UI 重新输入 API Key
  for (var name2 of Object.keys(OAI_COMPAT_PROVIDERS)) {
    if (enabledProviders.has(name2)) return name2;
  }
...
var server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && (req.url === "/v1/chat/completions" || req.url === "/chat/completions")) {
    ...
    try {
      ...
      if (configuredRoute) {
        body.model = configuredRoute.model;
        log.info(`[proxy] current config route: requested="${requestedModel || "<none>"}" -> ${provider}(${body.model}) source=${configuredRoute.source}`);
      } else {
        // 如果还是找不到提供商，使用默认提供商作为回退
        if (!provider) {
          var defaultProvider = getFallbackProvider();
