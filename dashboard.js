...
async function quickApply() {
  try {
    ...
    var configResult = await api('/api/codex-config', 'POST', configBody);
    if (!configResult || configResult.error) {
      toast('配置写入失败: ' + (configResult?.error || '未知错误'), 'error');
      return;
    }
    ...
    toast('配置应用失败: ' + (e.message || '未知错误'), 'error');
    console.error('[quickApply] Error:', e);
  }
}
